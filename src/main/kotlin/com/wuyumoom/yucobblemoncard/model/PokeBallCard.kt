...
import com.wuyumoom.yucobblemoncard.view.PokeBallGUI
...
class PokeBallCard (
    ...
    override fun execute(player: Player, item: ItemStack) {
        ...
        serverPlayer.openPartyWithCallback(
            ...
            if (isRandom){
                val random = YuPokeBall.getPokeBall().values.random()
                poke.caughtBall = random
                item.amount--
                ConfigManager.message.sendMessage("use",player)
            }else{
                val configuration = ConfigManager.viewConfigurationMap["PokeBallGUI"]?: return@openPartyWithCallback
                PokeBallGUI(player, configuration, item,this, poke,0).openInventory(player)
            }
        }
    }
}